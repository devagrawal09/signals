...
export function read<T>(el: Signal<T> | Computed<T>): T {
  if (context) {
    ...
    if ("fn" in el) {
      ...
      if (el.height >= stabilizeHeight || el.flags & (ReactiveFlags.Dirty | ReactiveFlags.Check)) {
        markHeap();
...
function markNode(el: Computed<unknown>, newState = ReactiveFlags.Dirty) {
  if ((el.flags & (ReactiveFlags.Check | ReactiveFlags.Dirty)) > newState) return;
  el.flags |= newState;
...
export function stabilize() {
  for (stabilizeHeight = 0; stabilizeHeight <= maxHeightInHeap; stabilizeHeight++) {
