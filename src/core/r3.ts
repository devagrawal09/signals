...
export function computed<T>(fn: (prev?: T) => T | Promise<T>, value?: T): Computed<T> {
  ...
  if (context) {
    if (context.depsTail === null) {
      ...
      recompute(self as Computed<unknown>, false);
    } else {
      self.height = context.height + 1;
      insertIntoHeap(self as Computed<unknown>);
    }
    link(self, context);
  } else {
    recompute(self as Computed<unknown>, false);
...
function setAsyncFlags(signal: Signal<unknown>, flags: AsyncFlags, error: Error | null = null) {
...
export function read<T>(el: Signal<T> | Computed<T>): T {
  if (context) {
    ...
    if ("fn" in el) {
      ...
      if (height >= minDirty || el.flags & (ReactiveFlags.Dirty | ReactiveFlags.Check)) {
        markHeap();
        updateIfNecessary(el as Computed<unknown>);
      }
    }
  }
